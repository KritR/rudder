...
import { loadTmuxDashboardState, saveTmuxDashboardState, } from "./tmux-state.js";
...
export function repoTmuxSessionName(repoRoot) {
...
export async function ensureTmuxDashboardSession(params) {
    ...
    const taskPaneId = (await runTmux([
        "split-window",
        "-v",
        "-t",
        agentPaneId,
    ...
    const workerPaneId = (await runTmux([
        "split-window",
        "-h",
        "-t",
        agentPaneId,
...
export async function normalizeTmuxDashboardLayout(repoRoot, sessionName) {
...
export async function configureRudderSession(sessionName) {
...
export async function paneExitStatus(paneId) {
...
export async function selectNextPane(sessionName) {
    await runTmux(["select-pane", "-t", `${sessionName}:0.+`], true);
}
export async function resizePane(paneId, height) {
...
async function paneId(sessionName, paneIndex) {
    const result = await runTmux(["display-message", "-p", "-t", `${sessionName}:${paneIndex}`, "#{pane_id}"]);
    return result.stdout.trim();
}
async function windowWidth(sessionName) {
