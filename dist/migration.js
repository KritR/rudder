 ...
 */
export function buildFreshHandoffPrompt(candidate, recentTurns) {
    const lines = [];
 ...
 */
export async function findMigrationCandidates(repoRoot) {
    const runsDir = path.join(repoRoot, ".rudder", "runs");
    const entries = await fsp.readdir(runsDir, { withFileTypes: true }).catch((err) => {
 ...
 */
export function encodeClaudeProjectsCwd(absolutePath) {
    return absolutePath.replace(/[^A-Za-z0-9-]/g, "-");
}
export function claudeSessionJsonlPath(cwd, sessionId) {
    return path.join(os.homedir(), ".claude", "projects", encodeClaudeProjectsCwd(path.resolve(cwd)), `${sessionId}.jsonl`);
}
/**
 ...
 */
export function cloudWorkspacePath(repoName) {
    return path.posix.join("/workspace", repoName);
}
export function cloudWorktreeAbsolutePath(repoName, runId, task) {
...
function cloudWorktreeDirName(runId, task) {
...
export function applyDefaultDecisions(candidates) {
...
export function summaryAsJson(plan) {
    return {
