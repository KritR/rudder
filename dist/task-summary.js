...
export function summarizeTask(task, maxChars = DEFAULT_MAX_CHARS) {
...
function compactTitle(value, maxChars) {
...
function cleanLlmTitleText(raw) {
    let text = raw;
...
function titleFromJsonOutput(raw) {
...
const SUMMARY_PROMPT_PREFIX = "Summarize this coding agent task for a compact sidebar label. Return exactly one JSON object and no markdown: {\"title\":\"5-8 word imperative title\"}. No quotes inside the title, no trailing punctuation.\n\nTask: ";
async function summarizeViaApiKey(task, apiKey) {
    try {
        const response = await fetch("https://api.anthropic.com/v1/messages", {
            ...
            body: JSON.stringify({
                ...
                max_tokens: 50,
                system: "Summarize coding tasks for compact sidebar labels. Return exactly one JSON object with a title string and no markdown.",
...
async function summarizeViaClaudeCli(task) {
    ...
    return await new Promise((resolve) => {
        ...
        let child;
        try {
            child = spawn("claude", ["-p", "--model", "claude-haiku-4-5-20251001"], { stdio: ["pipe", "pipe", "ignore"] });
        }
        catch {
            finish(null);
            return;
...
export async function llmSummarizeTask(task) {
