...
export async function discoverModelOptions(backend, configuredDefault) {
    const defaultDetail = configuredDefault || "CLI default";
    const options = [{ label: "Default", value: undefined, detail: defaultDetail }];
    if (backend === "claude") {
...
export function fallbackModelOptions(backend, configuredDefault) {
    const defaultDetail = configuredDefault || "CLI default";
    if (backend === "claude") {
        return [
...
async function discoverCodexModelsDev() {
...
async function discoverClaudeModelsDev() {
    const data = await readModelsDev();
...
async function discoverCodexModelsLocal() {
...
async function discoverClaudeModelsLocal() {
    ...
    const options = claudeCodeAliasOptions();
    for (const option of recent) {
        pushUnique(options, option);
    }
    return options;
}
async function readModelsDev() {
...
async function writeModelsDevCache(data) {
    const file = modelsDevCachePath();
...
function isUsableTextModel(id, model) {
    if (!model.tool_call) {
        return false;
    }
...
function claudeCodeAliasOptions() {
    return [
...
function scoreClaudeModel(id, model) {
    let score = 0;
...
function scoreCodexModel(id, model) {
    let score = 0;
...
function shortModelDetail(model) {
    const parts = [];
    if (model.name) {
...
async function collectClaudeProjectModels(dir, counts, depth = 0) {
...
function collectModelFields(value, counts, depth = 0) {
...
function prettyClaudeModel(model) {
...
function pushUnique(options, option) {
